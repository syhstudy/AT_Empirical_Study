...
class RNNAtten(nn.Module):
    def __init__(self, config):
        super(RNNAtten, self).__init__()
    ...
    def forward(self, x):
        # x = [batch size, sentence_len, hidden_size]
        output, (hidden, cell) = self.lstm(x)
        # output = [batch size, sentence_len, num_directions * hidden_size]
        M = self.tanh(output)
        # M = [batch size, sentence_len, num_directions * hidden_size]
        alpha = F.softmax(torch.matmul(M, self.w), dim=1).unsqueeze(-1)
        # alpha = [batch size, sentence_len, 1]
        out = output * alpha
        # print(alpha)
        # out = [batch size, sentence_len, num_directions * hidden_size]
        out = F.gelu(out)
        # out = [batch size, sentence_len, num_directions * hidden_size]
        out = self.dense(out)
        # out = [batch size, sentence_len, hidden_size]
        return out, alpha
...
class Seq2Seq(nn.Module):
    ...
    def __init__(self, encoder, decoder, config, beam_size=None, max_length=None, sos_id=None, eos_id=None):
        ...
        self.eos_id = eos_id
        #利用所有层信息
        self.semantic_attention = RNNAtten(config)
    ...
    def forward(self, source_ids=None, source_mask=None, target_ids=None, target_mask=None, args=None):
        outputs = self.encoder(source_ids, attention_mask=source_mask)[0]
        outputs = self.semantic_attention(outputs)
    ...
    def train(self, train_filename, train_batch_size, num_train_epochs, learning_rate,
        ...
        for epoch in range(num_train_epochs):
            ...
            for batch in bar:
                ...
                # m = 5
                # for t in range(m):
                #     free_at.attack(is_first_attack=(t == 0))  # 在embedding上添加对抗扰动, first attack时备份param.data
                #     if t != m - 1:
                #         self.model.zero_grad()
                #     else:
