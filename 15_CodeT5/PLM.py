...
class PLM_model():
        ...
        for cur_epoch in range(int(num_train_epochs)):
            ...
            if do_eval==True:
                ...
                if do_eval_bleu:
                    ...
                    pred_nls = [self.tokenizer.decode(id, skip_special_tokens=True, clean_up_tokenization_spaces=False) for
                                id in pred_ids]
                    hyp_list = []
                    ref_list = []
    ...
    def test(self, batch_size, filename, output_dir):
        ...
        eval_examples = read_examples(filename, 'test')
        ...
        metrics_dict = compute_metrics(hypothesis=os.path.join(output_dir, "test_hyp.csv"),
                                       references=[os.path.join(output_dir, "test_ref.csv")],
                                       no_skipthoughts=True, no_glove=True)
        bleu4 = round(metrics_dict['Bleu_4'], 4)
